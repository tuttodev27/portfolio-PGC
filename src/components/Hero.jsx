export default function Hero() {
  ...
  return (
    <section className="section-base min-h-screen relative overflow-hidden">
      ...
      {/* HERO — stacks verticalmente en móvil */}
      <div className="container-max pt-28 pb-10 md:pt-32 md:pb-16 grid md:grid-cols-[1fr_auto] gap-10 md:gap-16 items-center relative z-10">
        ...
        <div>
          ...
          <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-emerald-900/30 border border-emerald-500/20 mb-6">
          ...
          {/* Stats */}
          <div className="flex gap-6 md:gap-8 mb-8 pb-8 border-b border-white/5">
        ...
        {/* RIGHT — code card, solo en desktop */}
...
{"  "}<span className="text-blue-400">private final</span> List&lt;String&gt; core ={"\n"}
{"    "}List.of({"\n"}
      ...
      <div className="container-max pb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {stack.map(({ tech, level, progress }) => (
            <div key={tech} className="bg-slate-900/50 border border-slate-700 rounded-2xl p-5 transition-colors hover:border-slate-600">
              <div className="flex items-center justify-between mb-4 gap-3">
                <span className="text-sm font-semibold text-white">{tech}</span>
                <span className="text-[11px] uppercase tracking-[0.25em] text-slate-400 shrink-0">{level}</span>
              </div>
              <div className="h-2.5 rounded-full bg-slate-800 overflow-hidden">
                <div className={`h-full rounded-full ${getBarColor(progress)}`} style={{ width: `${progress}%` }} />
              </div>
            </div>
          ))}
        </div>
